...
async function _report_fetchCsrfToken() {
    try {
        console.log('Fetching CSRF token from:', _report_api_urls.dashboard);
        ...
        if (!_report_csrf_token) {
            ...
            console.log('CSRF token fetched successfully:', _report_csrf_token);
        }
    } catch (error) {
...
async function _report_fetchUsers() {
    try {
        console.log('Fetching users from:', _report_api_urls.users);
        const response = await fetch(_report_api_urls.users);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
        _report_users = data.results || [];
        console.log('Users fetched:', _report_users.length);
    } catch (error) {
...
async function _report_fetchEngagements() {
    try {
        console.log('Fetching engagements from:', _report_api_urls.engagements);
        const response = await fetch(_report_api_urls.engagements);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
        _report_engagements = data.results || [];
        console.log('Engagements fetched:', _report_engagements.length);
    ...
    if (_report_engagements.length > 0) {
        ...
        select.selectmenu({
            ...
            change: async function(event, ui) {
                ...
                console.log('Selected engagement ID:', _report_selected_engagement_id);
...
async function _report_fetchTests() {
    try {
        ...
        if (!buildResponse.ok) throw new Error(`HTTP error! status: ${buildResponse.status}`);
        ...
        if (!securityResponse.ok) throw new Error(`HTTP error! status: ${securityResponse.status}`);
...
$(document).ready(() => {
    _report_fetchCsrfToken().then(() => {
        console.log('CSRF token fetch completed, proceeding with users and engagements');
        _report_fetchUsers();
        _report_fetchEngagements();
    }).catch(error => {
        console.error('Initialization error:', error);
        _report_showToast('Initialization failed: ' + error.message, 'error');
    });
});